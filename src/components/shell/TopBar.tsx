...
type TopBarProps = { user: SessionUser; pendingQueue?: number };
 ...
 * a User's HTML never carries it (AC-07.3).
 */
export async function TopBar({ user, pendingQueue }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-40 bg-brand text-on-brand border-b-4 border-gold">
      <div className="max-w-page mx-auto px-4 md:px-6 flex items-center gap-3 md:gap-7 min-h-14.5 md:min-h-16 relative">
        ...
        <TopBarNav links={links} pathname={pathname} />
